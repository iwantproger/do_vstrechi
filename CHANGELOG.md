...
- `feat`: воронка пользователей — регистрация → создал расписание → получил бронирование (горизонтальные бары с %)
- `feat`: retention по когортам — Day 1 / Day 7 / Day 30, переключатель периодов, цветовые бейджи rate
- `feat`: тренд регистраций за 30 дней — фиолетовый line chart, отдельно от тренда бронирований
- `feat`: Time to Value — медиана часов до первого бронирования + distribution bar chart по бакетам
- `feat`: топ организаторов — таблица с расписаниями/встречами/последней активностью; privacy: имя + первая буква фамилии
- 5 новых API: `/api/admin/analytics/funnel`, `retention`, `organizer-stats`, `registrations-trend`, `time-to-value`
- Аналитика загружается отдельным try/catch — ошибка одного эндпоинта не ломает основной дашборд

### Added — Мультивыбор задач в Kanban

- `feat`: Ctrl/Cmd+клик = toggle выделения карточки, кнопка «Выбрать» = режим мультивыбора
- `feat`: плавающий тулбар с «Скопировать» + «Сбросить», Ctrl+A / Ctrl+C / Esc
- `feat`: копирование в Markdown — все выбранные задачи одним текстом
