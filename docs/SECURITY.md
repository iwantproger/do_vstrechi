...
| Секреты | `.env` only, git history очищена (filter-repo) | 2026-04-15 |
| Шифрование at-rest | Нет (принятый риск R4) | 2026-04-15 |
| Audit log | Нет (принятый риск R5) | 2026-04-15 |
| DDoS protection | nginx rate limit (принятый риск R6) | 2026-04-15 |
...
|---|----------|----------|--------|---------------|
| R1 | HIGH | Секреты в git history (commit 98a3d39 и ранее) — `POSTGRES_PASSWORD`, `BOT_TOKEN`, `SECRET_KEY` | ✅ ЗАКРЫТ 08.04.2026: ротация выполнена, git filter-repo очистил историю, force push в origin | DevOps |
...
| R4 | MEDIUM | Нет шифрования данных на диске (at-rest encryption) — данные в PostgreSQL хранятся в открытом виде. При физическом доступе к VPS данные пользователей (имена, контакты, время встреч) доступны без дополнительной защиты | Принятый риск для текущего масштаба. Митигация: БД не открыта наружу, доступ только через Docker-сеть, пароль ротируется. Future: рассмотреть pgcrypto для шифрования sensitive полей (guest_contact, notes) | Backend |
| R5 | MEDIUM | Нет audit log действий пользователей — если аккаунт организатора скомпрометирован, невозможно восстановить хронологию действий (что удалено, изменено, просмотрено) | Принятый риск. structlog логирует HTTP-запросы, но нет бизнес-уровня аудита. Future: добавить таблицу `audit_log` с записью CRUD-операций | Backend |
| R6 | LOW | Базовый DDoS-protection — rate limiting через nginx (10 req/s на API) достаточен для текущей нагрузки, но не защищает от распределённых атак на уровне L3/L4 | Принятый риск для текущего масштаба. Future при росте: Cloudflare или аналог перед nginx | DevOps |
...
## Аудит #3 — 2026-04-15 (мини-ревью: шифрование и инфраструктура)

**Аудитор:** Ручной анализ (разговор с командой)
**Scope:** Шифрование данных пользователей, защита от внешних атак
**Триггер:** Плановое ревью после внедрения beta-окружения

### Итог

Новых критических уязвимостей не найдено. Зафиксированы три осознанных
ограничения текущей архитектуры (R4, R5, R6) — все являются принятыми
рисками для стадии MVP и требуют внимания при масштабировании.

### Что работает хорошо

| Слой | Механизм |
|------|----------|
| Передача данных | HTTPS TLS 1.2/1.3 + HSTS — перехват невозможен |
| Аутентификация | HMAC-SHA256 Telegram initData — подделка identity исключена |
| SQL | Параметризованные запросы — SQL-инъекции исключены |
| XSS | `escHtml()` + CSP — XSS закрыт |
| Перебор ID | UUID v4 (122 бит) — угадать schedule_id нереально |
| Авторизация | Серверная изоляция по user_id — организатор видит только свои данные |
| Контейнеры | Non-root `appuser`, БД только в Docker-сети |
| Секреты | `.env` only, git history очищена (R1 закрыт) |
| Rate limiting | nginx: 10 req/s API, 5 req/min bookings |
| OAuth токены | Fernet/AES шифрование для Google Calendar токенов |

### Открытые риски после ревью

Добавлены R4 (at-rest encryption), R5 (audit log), R6 (DDoS).
Все три — принятые риски, не требующие немедленных действий.

### Следующий полный аудит

Рекомендуется при достижении 100+ активных пользователей
или перед добавлением платёжного функционала.
