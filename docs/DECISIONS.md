...
| Низкий | Два файла миграции 008 | `database/migrations/008_calendar_integration.sql`, `008_no_answer_status.sql` | Не влияет на работу (оба идемпотентны), но нарушает последовательную нумерацию. Переименовать один в 009 | Открыт |
