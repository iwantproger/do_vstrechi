...
## Двухсерверная архитектура (с 2026-05-02)

После инцидента с потерей связности Timeweb↔Telegram CDN ([INC-002](docs/incidents/INC_002_TIMEWEB_TELEGRAM_DISCONNECT.md)) Telegram-бот вынесен на отдельный VPS, всё остальное осталось на Timeweb.

| Компонент | VPS | IP | Что хранит |
|-----------|-----|-----|------------|
| **Backend + БД + nginx + frontend** | Timeweb (РФ) | 109.73.192.69 | Все пользовательские данные (152-ФЗ) |
| **Telegram-бот + Redis (FSM)** | ishosting (Финляндия) | 38.244.193.239 | Ничего не хранит — только пересылает |

**Коммуникация:**
- Бот → Backend: `https://dovstrechiapp.ru/api/*` + `X-Internal-Key` (HMAC verify в FastAPI)
- Backend → Бот: `http://10.0.0.2:8080/internal/*` через **WireGuard-туннель** + `X-Internal-Key`
- Бот → Telegram: `https://api.telegram.org` напрямую (foreign route, polling-режим)

**WireGuard:**
- Туннель `10.0.0.0/24` между Timeweb (`10.0.0.1`) и ishosting (`10.0.0.2`), UDP 51820
- Бот биндит `8080` ТОЛЬКО на `10.0.0.2` (`BOT_BIND_HOST=10.0.0.2` в `.env` бота). Снаружи `38.244.193.239:8080` недоступен — `ufw deny`
- Конфиги: `/etc/wireguard/wg0.conf` (chmod 600) + `/etc/wireguard/{timeweb,finland}_priv.key`. Public ключи можно публиковать
- Управление: `systemctl status/restart wg-quick@wg0`, `wg show wg0`

**152-ФЗ:** соблюдается — все persistent данные (пользователи, расписания, бронирования, токены календарей) на РФ-VPS. Бот в Финляндии работает чистым прокси Telegram-сообщений, ничего не сохраняет.

**TODO архитектуры:** WireGuard backend↔bot, CI/CD для финского VPS (сейчас деплой бота ручной через `scp + docker compose build`), внешний uptime-мониторинг.
...
| Ветка | `dev` | `main` |
| Бот (TG) | @beta_do_vstrechi_bot | @do_vstrechi_bot |
| Бот (хостинг) | Timeweb worktree (`docker-compose.beta.yml`) | **ishosting Финляндия** (`/opt/dovstrechi-bot`, отдельный compose) |
| БД | dovstrechi_beta | dovstrechi (Timeweb) |
| Путь backend на VPS | `/opt/dovstrechi-beta` (worktree, Timeweb) | `/opt/dovstrechi` (Timeweb) |
| Compose файл | docker-compose.beta.yml | docker-compose.yml + docker-compose.bot.yml (на финском) |
| Деплой backend | Автоматически при push в `dev` | Только через процедуру |
| Деплой бота | Через основной compose (на Timeweb) | **Ручной** (scp + `docker compose up -d` на 38.244.193.239) |
| CI/CD workflow | `deploy-beta.yml` | `deploy-prod.yml` (только backend) |
...
| GET | `/api/bookings/pending-reminders-v2` | internal | Напоминания по user.reminder_settings + sent_reminders |
...
| POST | `/api/sent-reminders` | internal | Записать отправленное напоминание (idempotent) |
...
| `bookings` | id (UUID PK), schedule_id (FK→schedules CASCADE), guest_name, guest_contact, guest_telegram_id, scheduled_time (TIMESTAMPTZ), status (CHECK: pending/confirmed/cancelled/completed/**no_answer**), meeting_link, notes, **title** (TEXT), **end_time** (TIMESTAMPTZ), **is_manual** (BOOL), **created_by** (BIGINT), **platform** (TEXT, snapshot), **location_address** (TEXT, snapshot), **blocks_slots** (BOOL, default TRUE), **confirmation_asked** (BOOL), **confirmation_asked_at** (TIMESTAMPTZ), created_at, updated_at | Бронирования встреч |
...
### Timeweb (РФ, 109.73.192.69) — backend + БД + nginx + frontend

| Параметр | Значение |
|----------|---------|
| Провайдер | Timeweb (Россия, 152-ФЗ) |
...
### ishosting (Финляндия, 38.244.193.239) — Telegram-бот

| Параметр | Значение |
|----------|---------|
| Провайдер | ishosting, VPS #406232, Helsinki |
| OS | Ubuntu 22.04 LTS |
| Деплой-путь | `/opt/dovstrechi-bot` |
| Compose файл | `docker-compose.bot.yml` (в репо, на VPS переименован в `docker-compose.yml`) |
| .env | На VPS, секреты совпадают с Timeweb для `BOT_TOKEN`, `INTERNAL_API_KEY`, `ADMIN_TELEGRAM_*` |
| Firewall (ufw) | 22/tcp anywhere, 51820/udp anywhere (WireGuard), остальное deny. **8080 не доступен публично** — биндится только на `10.0.0.2` (WG interface) |
| SSH alias | `dovstrechi-bot` (см. `~/.ssh/config`), ключ `~/.ssh/dovstrechi_deploy` (тот же, что для Timeweb) |
| Деплой | Ручной (см. ниже) |

**Ручной деплой бота на финский VPS** (пока нет CI workflow):
```bash
# С локальной машины:
tar --exclude='__pycache__' -czf /tmp/bot-deploy.tar.gz bot/ docker-compose.bot.yml
scp /tmp/bot-deploy.tar.gz root@38.244.193.239:/tmp/
ssh root@38.244.193.239 'cd /opt/dovstrechi-bot && tar xzf /tmp/bot-deploy.tar.gz && \
  rm -f docker-compose.yml && mv docker-compose.bot.yml docker-compose.yml && \
  docker compose build && docker compose up -d && docker compose logs --tail=30 bot'
```

### Сервисы в docker-compose (Timeweb)

| Сервис | Контейнер | Порты | Примечание |
|--------|----------|-------|------------|
| postgres | dovstrechi_postgres | — (internal) | |
| backend | dovstrechi_backend | 8000 (internal) | `BOT_INTERNAL_URL=http://38.244.193.239:8080` |
| **~~bot~~** | ~~dovstrechi_bot~~ | — | **Archived** (`profiles: ["archived"]`), переехал на ishosting |
| nginx | dovstrechi_nginx | 80, 443 (external) | `/bot/webhook` закомментирован, geo-whitelist финского VPS для booking rate-limit |
| redis | dovstrechi_redis | — (internal) | |
| certbot | dovstrechi_certbot | — (профиль `ssl`) | |
| support-bot | dovstrechi_support_bot | — (internal, только в beta) | |

### Сервисы в docker-compose.bot.yml (ishosting)

| Сервис | Контейнер | Порты |
|--------|----------|-------|
| bot | dovstrechi_bot | 8080 (внешний — для backend pushes от Timeweb) |
| redis | dovstrechi_bot_redis | — (internal, FSM storage) |
...
- **Напоминания** — фоновый цикл в боте (`reminder_loop`, каждые 60с) проверяет `pending-reminders-v2` через API и рассылает по настройкам `users.reminder_settings`. Дедупликация через таблицу `sent_reminders`
...
- **Напоминания** используют таблицу `sent_reminders` (дедупликация) + JSONB `users.reminder_settings` (произвольные интервалы). Legacy V1 boolean-флаги удалены (migration 017)
